...
function selectItems() {
	...
	$.ajax({
		type : "get",
		dataType : 'json',
		url : 'editGeneralDetail/' + string[0],
		contentType : "application/json",
							...
							<tr>
								<td>
									<table class="table table-bordered">
										...
										<tbody>
											<c:forEach items="${generalMap}" var="getGeneral">
												<c:if test="${getGeneral.key == 'generalList'}">
													<c:forEach items="${getGeneral.value}" var="general">
														<tr>
															<td><input type="checkbox" id="check" name="check" value="${general.guid}"></td>
															<td>${general.companyName}</td>
															<td>${general.companyPhone}</td>
															<td>${general.contactName}</td>
															<td>${general.contactMobile}</td>
															
<!-- 														試用帳號數量 -->
															<c:forEach items="${generalMap}" var="getCount">
																<c:if test="${getCount.key == 'tryonCountMap'}">
																	<c:forEach items="${getCount.value}" var="count">
																		<c:if test="${count.key == general.guid}">
																			<td>${count.value}</td>
																		</c:if>
																	</c:forEach>
																</c:if>
...
<!-- 														代理商數量 -->
															<c:forEach items="${generalMap}" var="getCount">
																<c:if test="${getCount.key == 'agentsCountMap'}">
																	<c:forEach items="${getCount.value}" var="count">
																		<c:if test="${count.key == general.guid}">
																			<td>${count.value}</td>
																		</c:if>
		...
		<div class="modal-dialog">
			...
			<div class="modal-content">
				...
				<div class="modal-body scrollbar">
					...
					<button type="button" class="btn btn-default" onclick="editAgentsView()">代理商</button>
					<hr class="divider1">
					<div class="input-group">
						<form class="form-horizontal" role="form">
